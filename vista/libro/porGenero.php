...
<!--        <script src="../js/registrarse.js"></script>-->
    ...
    <body>
        ...
        if (!$_GET['pag']) {
            header('Location: porGenero.php?pag=1');
        }
        if (!isset($_SESSION['letra']) and $_SESSION['letra'] != NULL) {
            $contenidoPorPag = 3;
            $w = $_SESSION['totalLibro'];
            $pagTotal = count($w);
            $paginas = ceil($pagTotal / $contenidoPorPag);
            $ini = ($_GET['pag'] - 1) * $contenidoPorPag;
            $l = Conexion::obtenerPorGeneroLimit($ini);
        } else {
            $letra = $_SESSION['letra'];
            if (!strcmp($letra, 'genero') == 0) {//cuando quieres buscar por una letra
                $m = Conexion::obtenerGenero($letra);
                if (!$m == NULL) {
                    $contenidoPorPag = 3;
                    $pagTotal = count($m);
                    $paginas = ceil($pagTotal / $contenidoPorPag);
                    $ini = ($_GET['pag'] - 1) * $contenidoPorPag;
                    $l = Conexion::obtenerGeneroLimit($ini, $letra);
                } else {
                    header('Location: porGenero.php?pag=1');
                }
            } else {
            ...
            <div class="row">
                <div class="col-sm col-md col-lg">
                    <div class="breadcrumb">
                        ...
                        <div class="breadcrumb-item active">Por Género</div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm col-md col-lg mt-3 mb-3">
                    <h2 class="text-center">LIBROS ORDENADOS POR GENERO</h2>
                    <form class="form-inline" action="../../controlador/controladorLibro.php" method="POST">
                        <div class="form-group-sm form-group-md form-group-lg mt-3 mb-3">
                            ...
                            <input type="submit" class="btn btn-primary" name="filtrarG" value="Filtrar">
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                <form class="form-inline" action="../../controlador/controladorLibro.php" method="POST">
                                    <tr>
                                        <td><img id="fotoLibro" class="img-fluid" src="data:image/jpeg;base64,<?php echo base64_encode($key->getFoto()); ?>" alt='Foto no encontrada'/>
                                                            ...
                                                            if (isset($_SESSION['u'])) {
                                                                ...
                                                                <form class="form-inline" action="../../controlador/controladorGeneral.php" method="POST">
                                                                    <div class="form-group mt-3 mb-3">
                                                                        ...
                                                                        <input type="hidden"  name="fotoUsuario" value="data:image/jpeg;base64,<?php echo base64_encode($u->getFoto()); ?>">
                                                                    </div>
                                                                    <div class="form-group mt-3 mb-3">
                                                                        <label><?php echo $u->getNombre(); ?></label> 
                                                                        <h4><small><i>Posted on <input type="text" style="border:none" name="fechaNoticia"/></i></small></h4>
                                                                    </div>
                                                                    <div class="form-group mt-3 mb-3">
                                                                        <label>Asunto: </label>
            ...
            <nav aria-label="pagination">
                <ul class="pagination">
                    <li class="page-item <?php echo $_GET['pag'] <= 1 ? 'disable' : '' ?>">
                        <a class="page-link" href="porGenero.php?pag=<?php $_GET['pag'] - 1; ?>">Previus</a>
                    </li>
                    <?php
                    for ($i = 0; $i < $paginas; $i++) {
                        ?>
                        <li class="page-item <?php echo $_GET['pag'] == $i + 1 ? 'active' : '' ?>">
                            <a class="page-link" id="pagina" href="porGenero.php?pag=<?php echo $i + 1; ?>"><?php echo $i + 1; ?></a>
                        </li>
                        <?php
                    }
                    ?>
                    <li class="page-item <?php echo $_GET['pag'] >= $paginas ? 'disable' : '' ?>">
                        <a class="page-link" href="porGenero.php?pag=<?php echo$_GET['pag'] + 1; ?>">Next</a>
                    </li>
                </ul>
            </nav>
            <?php
