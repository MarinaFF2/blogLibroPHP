...
<!--        <script src="../js/registrarse.js"></script>-->
    ...
    <body>
        ...
        if (!$_GET['pag']) {
            header('Location: porNombre.php?pag=1');
        }
        if (!isset($_SESSION['letra']) and $_SESSION['letra'] != NULL) {
            ...
            $l = Conexion::obtenerPorNombreLimit($ini);
        } else {
            $letra = $_SESSION['letra'];
            if (!strcmp($letra, 'nombre') == 0) {//cuando quieres buscar por una letra
                $m = Conexion::obtenerNombre($letra);
                if (!$m == NULL) {
                    $contenidoPorPag = 3;
                    $pagTotal = count($m);
                    $paginas = ceil($pagTotal / $contenidoPorPag);
                    $ini = ($_GET['pag'] - 1) * $contenidoPorPag;
                    $l = Conexion::obtenerNombreLimit($ini, $letra);
                } else {
                    header('Location: porNombre.php?pag=1');
                }
            } else {
            ...
            <div class="row">
                <div class="col-sm col-md col-lg">
                    <div class="breadcrumb">
                        ...
                        <div class="breadcrumb-item active">Por nombre</div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm col-md col-lg mt-3 mb-3">                    
                    <h2 class="text-center">LIBROS ORDENADOS POR NOMBRE</h2>
                    <form class="form-inline" action="../../controlador/controladorLibro.php" method="POST">
                        <div class="form-group mt-3 mb-3 ">
                            ...
                            <input type="submit" class="btn btn-primary" name="filtrarN" value="Filtrar">
                        </div>
                    </form>
                </div>
            </div> 
                                                            ...
                                                            if (isset($_SESSION['u'])) {
                                                                ...
                                                                <form class="form-inline" action="../../controlador/controladorGeneral.php" method="POST">
                                                                    ...
                                                                    <div class="form-group mt-3 mb-3">
                                                                        ...
                                                                        <h4><small><i>Posted on <input type="text" class="form-control form-control-sm form-control-md form-control-lg" style="border:none" name="fechaNoticia"/></i></small></h4>
                                                                    </div>
                                                                    <div class="form-group mt-3 mb-3">
                                                                        <label>Asunto: </label>
        ...
        <nav aria-label="pagination">
            <ul class="pagination">
                <li class="page-item <?php echo $_GET['pag'] <= 1 ? 'disable' : '' ?>">
                    <a class="page-link" href="porNombre.php?pag=<?php $_GET['pag'] - 1; ?>">Previus</a>
                </li>
                <?php
                for ($i = 0; $i < $paginas; $i++) {
                    ?>
                    <li class="page-item <?php echo $_GET['pag'] == $i + 1 ? 'active' : '' ?>">
                        <a class="page-link" id="pagina" href="porNombre.php?pag=<?php echo $i + 1; ?>"><?php echo $i + 1; ?></a>
                    </li>
                    <?php
                }
                ?>
                <li class="page-item <?php echo $_GET['pag'] >= $paginas ? 'disable' : '' ?>">
                    <a class="page-link" href="porNombre.php?pag=<?php echo$_GET['pag'] + 1; ?>">Next</a>
                </li>
            </ul>
        </nav>
        <?php
